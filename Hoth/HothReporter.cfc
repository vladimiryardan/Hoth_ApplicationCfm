	...
	private struct function generateExceptionIndex() {
		...
		for (i=1;i LTE ArrayLen(local.exceptions);i=i+1) {
			...
			if (left(local.instance.filename, 1) != '_')
			{
				...
				local.instance.dtunformatted = listfirst(local.instances,chr(13) ); 
				local.instance.dt = datetimeFormat( listfirst(local.instances,chr(13) ), "mmmm, dd yyyy HH:nn:ss" ) ;//this is buggy. should be HH:nn:ss instead of HH:mm:ss
				//local.instance.dt = dateFormat( listfirst(local.instances,chr(13) ), "mmmm, dd yyyy" ) & " " & timeFormat( listfirst(local.instances,chr(13) ), "HH:mm:ss" );
				//expected format February, 24 2016 17:48:51
